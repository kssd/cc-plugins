...
import { dataDir, ensureDir, listFiles, readMaybe } from "../util.js";
async function buildSurface(root) {
...
export const driftDetectorTools = [
    {
        ...
        handler: async (args) => {
            ...
            const snap = { takenAt: new Date().toISOString(), symbols };
    ...
    },
    {
        ...
        handler: async (args) => {
            ...
            if (!raw)
                return { error: "no snapshot — run drift_detector.snapshot first" };
            const prev = JSON.parse(raw);
            const curr = await buildSurface(root);
            const key = (e) => `${e.file}::${e.symbol}`;
            const prevMap = new Map(prev.symbols.map(e => [key(e), e]));
            ...
            for (let i = removed.length - 1; i >= 0; i--) {
                const r = removed[i];
                const candIdx = added.findIndex(a => a.kind === r.kind && (a.symbol === r.symbol /* moved */ ||
            ...
            if (args.mode === "advisory") {
                ...
                if (flagged.length === 0 && signatureChanged.length === 0)
