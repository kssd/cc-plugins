...
export const specStoreTools = [
    ...
    },
    {
        ...
        handler: async (args) => {
            ...
            const feature = await resolveFeature(root, args.feature);
    ...
    },
    {
        ...
        handler: async (args) => {
            const root = args.root ?? process.cwd();
            const target = path.join(root, SPECS, args.feature, `${args.artifact}.md`);
            await fs.appendFile(target, "\n" + args.append + "\n", "utf8");
            return { path: target, appended: true };
        }
    }
];
async function resolveFeature(root, hint) {
    const specsDir = path.join(root, SPECS);
    ...
    if (!match)
        throw new Error(`feature not found: ${hint}`);
    return match;
}
