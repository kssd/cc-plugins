...
import { dataDir, ensureDir, listFiles, reqIdRegex, readMaybe } from "../util.js";
...
async function buildGraph(root) {
    ...
    for (const f of taskFiles) {
        ...
        while ((m = taskRe.exec(text)) !== null) {
            ...
            for (const r of reqs)
                edges.push({ from: tid, to: r, rel: "satisfies" });
        }
    }
    ...
    for (const f of codeFiles) {
        ...
        for (const r of ids)
            edges.push({ from: nid, to: r, rel: "implements" });
    }
...
export const traceabilityTools = [
    ...
    },
    {
        ...
        handler: async (args) => {
            ...
            if (!raw)
                return { error: "index not built — run traceability_graph.build first" };
            const g = JSON.parse(raw);
            let target = g.nodes;
            if (args.id)
                target = target.filter(n => n.id === args.id);
            if (args.file)
                target = target.filter(n => n.file && n.file.includes(args.file));
            if (args.symbol)
                target = target.filter(n => n.symbol === args.symbol);
            if (args.kind)
                target = target.filter(n => n.kind === args.kind);
            const out = target.map(n => {
                const adj = g.edges.flatMap(e => e.from === n.id ? [{ rel: e.rel, dir: "out", node: g.nodes.find(x => x.id === e.to) }]
...
//# sourceMappingURL=traceability-graph.js.map