...
import { readMaybe, type ToolDef } from "../util.js";

export const constitutionLookupTools: ToolDef[] = [
  {
    ...
    handler: async (args) => {
      ...
      if (!content) return { error: "constitution not found — run /sdd:constitution" };
      const version = content.match(/\*\*Version:\*\*\s*([^\n]+)/)?.[1]?.trim() ?? "unknown";
      const principleRe = /^### (P\d+) — (.+)$/gm;
      const principles: { id: string; name: string; body: string }[] = [];
      ...
      const kws = (args.keywords as string[] | undefined)?.map(k => k.toLowerCase()) ?? [];
